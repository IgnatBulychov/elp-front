    ...
    <v-card-text>
      ...
      >
        <v-file-input
            ...
            :messages="errors.newFiles.message"
            :error="errors.newFiles.status"
            :persistent-hint="errors.newFiles.status"
            :disabled="$store.state.file.isLoading"
          ...
          <v-btn
            class="mx-2"
            :loading="$store.state.file.loading"
    ...
    export default {
        ...
        methods: {
          ...
          validate () {
            ...
            if (!(this.errors.newFiles.status)) {
              return true
            } else {
              return false
            }
        ...
        if (this.newFiles.length == 0) {
          this.errors.newFiles.status = true
          this.errors.newFiles.message = 'Вы не добавили фото'
        } else if (this.newFiles.length > 15) {
          this.errors.newFiles.status = true
          this.errors.newFiles.message = 'Количество фотографий должно быть менее 15'
        } else if (max > 3000000) {
          this.errors.newFiles.status = true
          this.errors.newFiles.message = 'Изображения должны быть не более 3 Мб'
        } else if (errorsOfFormat > 0) {
          this.errors.newFiles.status = true
          this.errors.newFiles.message = 'Изображения должны быть в формате JPEG, PNG, BMP, GIF или SVG'
        } else { 
          this.validateImagesReset() 
        }
      },
      validateImagesReset() {
        this.errors.newFiles.status = false
        this.errors.newFiles.message = ''
      },
      remove(id, index) {
